...
function buildZakiySystemPrompt(memory: ZakiyMemoryData): string {
  ...
  return `⛔⛔⛔ قاعدة أولى مطلقة — قبل أي شيء:
لا تبدأ ردّك بالإعلان عن النبرة أو أسلوب القراءة أبداً.
لا تقل: "هقرألك النص بطريقه هاديه" / "سأقرأ بنبرة هادئة" / "حاضر هاتكلم بصدق" أو أي جملة مشابهة.
الفلتر التقني سيحذف هذه الجمل — فأنت تضيع كلاماً مش هيوصل للمستخدم. ابدأ مباشرة بالمحتوى.
⛔⛔⛔

أنت "الزكي" — مش بوت ولا شيخ رسمي. أنت **الأخ الأكبر الحكيم** — الواحد اللي بتتكلم معاه بصدق لأنه بيحبك ويخاف عليك، ومش هيسيبك تغلط وتسكت.
...
const ZAKIY_TTS_SYSTEM = `أنت تقرأ نصاً عربياً. اقرأه كما هو حرفياً بالضبط — لا تزيد ولا تنقص ولا تغيّر كلمة.
صوتك: رجل عربي خليجي، نبرة هادئة وثقيلة وواضحة، إيقاع متدفق طبيعي.
تحدّث بوضوح تام — لا تُلحلح ولا تبلع الكلمات. كل حرف يُنطق بوضوح.
لا تعلّق على النص ولا تضيف مقدمة — ابدأ القراءة مباشرة.`;
...
function stripToneAnnouncements(text: string): string {
  // ── Pass 1: Remove lines that explicitly announce reading style ──
  // Covers cases like: "هقرألك النص بطريقه هاديه" / "سأقرأ بنبرة هادئة" / "حاضر هاتكلم بصدق"
  const TONE_ANNOUNCE_PATTERN =
    /^[^\n]*(?:هقرأ|هاقرأ|هقرأ|سأقرأ|رح أقرأ|راح أقرأ|بقرأ|سأتكلم|هتكلم|هاتكلم|سأحكي|هحكي|سأرد|هرد|حاضر|تمام|ماشي|أوكي|أوكيه|طيب)[^\n]*(?:بنبر[ةه]|بأسلوب|بطريق[ةه]|بصوت|بهدوء|بصدق|بحماس|بدفء|بحنان|بتأمل|هادئ[ةه]?|حكيم[ةه]?|جاد[ةه]?|مباشر[ةه]?|كما طلبت|زي ما طلبت|زي ما قلت|كما أردت|على طلبك)[^\n]*$/gim;

  // ── Pass 2: Remove any line that announces "I will read this text" (without tone qualifier) ──
  // Covers: "هقرألك النص" / "سأقرأ لك هذا الكلام"
  const META_READ_PATTERN =
    /^[^\n]{0,40}(?:هقرأ|هاقرأ|سأقرأ|راح أقرأ|رح أقرأ)[^\n]{0,10}(?:ل[كك]|ليك|لي)[^\n]{0,50}(?:النص|الكلام|ده|هذا|هاد)[^\n]*$/gim;

  return text
    .replace(TONE_ANNOUNCE_PATTERN, "")
    .replace(META_READ_PATTERN, "")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
}
