...
import { Users, Plus, Check, BookOpen, Trash2, Share2, Trophy, ChevronDown, ChevronUp, X } from "lucide-react";
...
function loadKhatmas(): Khatma[] {
  try { return JSON.parse(localStorage.getItem("quran_khatmas") || "[]"); } catch { return []; }
}
function saveKhatmas(k: Khatma[]) { localStorage.setItem("quran_khatmas", JSON.stringify(k)); }
function genId() { return Date.now().toString(36) + Math.random().toString(36).slice(2); }

// ─── Create Khatma Modal ──────────────────────────────────────────────────────

function CreateKhatmaModal({ onClose, onCreate }: { onClose: () => void; onCreate: (k: Khatma) => void }) {
  ...
  return (
    ...
    >
      <motion.div
        ...
        dir="rtl"
      >
        <div className="w-10 h-1 rounded-full mx-auto mb-4" style={{ background: "rgba(139,92,246,0.3)" }} />
        <h3 className="font-bold text-base mb-4 text-center">إنشاء ختمة جماعية</h3>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            <p className="text-[11px] text-muted-foreground mb-2">الأعضاء ({members.length})</p>
            <div className="flex flex-wrap gap-2 mb-2">
              {members.map(m => (
                <div key={m} className="flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold"
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-xl text-sm bg-transparent outline-none text-right"
...
function KhatmaCard({ khatma, onUpdate, onDelete }: { khatma: Khatma; onUpdate: (k: Khatma) => void; onDelete: () => void }) {
      ...
      <div className="p-4">
        <div className="flex items-start gap-3 mb-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
            style={{ background: isComplete ? "rgba(34,197,94,0.2)" : "rgba(139,92,246,0.15)", border: "1px solid rgba(139,92,246,0.2)" }}>
            {isComplete ? <Trophy size={18} style={{ color: "#22c55e" }} /> : <BookOpen size={18} style={{ color: "#8b5cf6" }} />}
          </div>
          <div className="flex-1">
            <p className="font-bold text-sm">{khatma.name}</p>
            <p className="text-[10px] text-muted-foreground">{khatma.members.length} أعضاء · {new Date(khatma.createdAt).toLocaleDateString("ar-SA")}</p>
          </div>
        ...
        <div className="mb-2">
          <div className="flex justify-between mb-1">
            <span className="text-[10px] text-muted-foreground">{done}/{total} جزء</span>
            <span className="text-[10px] font-bold" style={{ color: isComplete ? "#22c55e" : "#8b5cf6" }}>{pct}%</span>
          </div>
          <div className="h-2 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.05)" }}>
            <motion.div
              ...
              style={{ background: isComplete ? "linear-gradient(90deg, #22c55e, #16a34a)" : "linear-gradient(90deg, #8b5cf6, #7c3aed)" }}
            />
          </div>
        </div>
                ...
                <div key={member.name}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] text-muted-foreground">
                      ({member.juz.filter(j => khatma.completedJuz[j]).length}/{member.juz.length} جزء)
                    </span>
                  </div>
...
export default function QuranKhatmaPage() {
  ...
  useEffect(() => { setKhatmas(loadKhatmas()); }, []);
  ...
  return (
    <div className="min-h-screen pb-24" dir="rtl">
      ...
      <div className="px-4 pt-4 flex flex-col gap-4">
        ...
        <div className="rounded-2xl p-4 flex items-center gap-3"
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground leading-snug">وزّع الأجزاء على أصدقائك وتتبعوا التقدم معاً حتى اكتمال الختمة</p>
          </div>
        </div>
        ...
        {khatmas.length === 0 ? (
          <div className="text-center py-12 flex flex-col items-center gap-3">
            ...
            <p className="text-[11px] text-muted-foreground">أنشئ أول ختمة جماعية مع أصدقائك</p>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {khatmas.map(k => (
      ...
      <AnimatePresence>
        {showCreate && (
          <CreateKhatmaModal onClose={() => setShowCreate(false)} onCreate={createKhatma} />
        )}
      </AnimatePresence>
    </div>
  );
